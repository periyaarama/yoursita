                ...
                <div class="columns-1 sm:columns-2 md:columns-3 gap-6 space-y-6">
                    @foreach ($images as $img)
                        @include('components._portfolio-card', [
                            'image' => $img->url,
                            'title' => 'Half-Up Hairstyle',
                            'id' => $img->id,
                            'type' => 'halfuphairstyle'
                        ])
                    @endforeach
                </div>
                ...
                @auth
                    @if(Auth::user()->hasRole('admin'))
                        <div class="flex justify-center mt-10">
                            <form action="{{ route('admin.halfuphairstyle.upload') }}" method="POST" enctype="multipart/form-data" id="uploadForm">
                                @csrf
